...
def union_find_tiled(image, tile_size):    
	...
	# OpenCL setup (zoals in jouw originele code)
	...
	kernel_union_within_tile_memory_optimized = build_kernel("union_within_tile_local_memory_optimized.cl")
	...
	local_mem = cl.LocalMemory(4 * tile_size * tile_size)
	work_group_shape = (tile_size, tile_size)
	
	# Hier doe ik padding om te gevens van de tegel op te vullen tot een volledige tegelgrootte
	g_width = (width + tile_size - 1) // tile_size * tile_size
	g_height = (height + tile_size - 1) // tile_size * tile_size
	global_work_shape = (g_width, g_height)
	...
	while True:
		...
		if iteration == 0:
			# EERSTE ITERATIE: Gebruik de snelle __local memory kernel. Jouw code was hier al correct.
			kernel_union_within_tile_memory_optimized.union_within_tile_optimized(
				queue, global_work_shape, work_group_shape,
				mask_buf, parent_buf, np.int32(width), np.int32(height),
				np.int32(tile_size), changes_buf, local_mem
			)
		else:
			# VOLGENDE ITERATIES: Gebruik de simpele globale kernel.
			kernel_union_within_tile.union_within_tile(
				queue, (width, height), None,  # CORRECTIE: Gebruik (width, height) en local_size=None
				mask_buf, parent_buf, np.int32(width), np.int32(height),
				np.int32(tile_size), changes_buf  # CORRECTIE: Geen local_mem argument
			)		

		kernel_union_horizontal_borders.union_horizontal_borders(
			queue, (width, height), None, mask_buf, parent_buf, np.int32(width), np.int32(height), np.int32(tile_size), changes_buf
		)
	...
	# De return value zoals je die verwachtte
...
def main():
	# Bepaal geldige tile sizes voor de specifieke GPU om crashes te voorkomen.
	# Dit is de enige check die essentieel is om te behouden.
	device = cl.get_platforms()[0].get_devices(cl.device_type.GPU)[0]
	max_work_group_size = device.max_work_group_size
	max_local_mem_size = device.local_mem_size
	print("--- GPU Limieten ---")
	print(f"Max work-group size: {max_work_group_size} items")
	print(f"Max local memory: {max_local_mem_size / 1024:.0f} KB")
	print("--------------------")
	possible_tile_sizes = [4, 8, 16, 32, 64, 128]
	valid_tile_sizes = [ts for ts in possible_tile_sizes if ts*ts <= max_work_group_size and ts*ts*4 <= max_local_mem_size]
	print(f"Geldige tile sizes voor deze GPU: {valid_tile_sizes}\n")

	# Jouw originele benchmark-lus
	for tile_size in valid_tile_sizes:
		print(f"--- Processing met Tile Size: {tile_size} ---")
		for image_path in IMAGES:
			image_name = image_path.rsplit("/", maxsplit=1)[-1].split(".")[0]
			output_file = f"2_2_{image_name}_tilesize_{tile_size}_.txt"
			...
			with open(output_file, "w") as f_out:
				...
				for run in runs:
					...
					# Jouw originele functie-aanroep
					...
					f_out.write(f"{elapsed:.4f}\n")
					print(f"Run {run + 1}/{len(runs)}: {elapsed:.4f}s")
